...
export default function AgentLog({ events, onComplete }) {
  ...
  return (
    <div className="flex-1 overflow-hidden bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col">
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        ...
        <p className="text-sm text-gray-500 mt-1">Live streaming from ADK pipeline</p>
      </div>
      
      <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-3" role="log" aria-live="polite">
        {events.length === 0 && (
        ...
        {events.map((event, index) => (
          <div key={`${event.step}-${index}`} className="animate-slide-in">
            <div className={`flex items-start gap-3 p-3 rounded-lg border ${stepColors[event.step] || 'bg-gray-100 text-gray-800 border-gray-200'}`}>
              <div className="flex-shrink-0 mt-0.5">
                {getStatusIcon(event.status)}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium capitalize">{stepLabels[event.step] || event.step}</span>
